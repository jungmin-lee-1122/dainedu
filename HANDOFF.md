...
- 관리자 사진 업로드는 `@vercel/blob/client`의 **Client Upload** 방식입니다. 사진 본문을
  Vercel Function으로 보내지 않고 브라우저에서 Public Blob으로 직접 전송하므로 4.5MB 함수 제한을
  받지 않습니다. `/api/upload`은 관리자 로그인 확인 후 토큰만 발급하며, 이미지당 최대 25MB입니다.
...
- 모바일은 KV 내용을 세로 중앙으로 재배치하고, 상단 섹션 탭을 한 줄 가로 스크롤 + 안내 문구로 구성했습니다.
- 모바일 AI 분석 4단계(`.wt-flow`)는 네 카드가 한 화면에 보이도록 4열로 압축했습니다.
- A DAY 시간표 위에는 좌우 스크롤 안내가 표시되며, 하단 고정바는 개강일·교육 기간을 한 줄로 두고 예약 버튼을 아래 중앙에 배치합니다.
- 학습 공간 사진은 모바일에서 해당 섹션이 보일 때만 3.4초 간격으로 자동 전환되고, 사용자가 터치하면 잠시 멈춥니다.
